...
const api: AsteriaApi = {
  ...
  searchBrowserFiles: (query) => ipcRenderer.invoke('browser:search-files', query),
  listFavoriteFiles: () => ipcRenderer.invoke('browser:list-favorites'),
  setFileFavorite: (fileId, favorite) => ipcRenderer.invoke('file:set-favorite', fileId, favorite),
  openDatabaseManagerWindow: () => ipcRenderer.invoke('window:open-database-manager'),
  ...
  openUrlManagerWindow: (fileIds) => ipcRenderer.invoke('window:open-url-manager', fileIds),
  openBatchTagManagerWindow: (fileIds) => ipcRenderer.invoke('window:open-batch-tag-manager', fileIds),
  openExportWindow: (fileIds) => ipcRenderer.invoke('window:open-export', fileIds),
  openScreeningWindow: (fileIds) => ipcRenderer.invoke('window:open-screening', fileIds),
  openFileDetailWindow: (id, sequenceIds) => ipcRenderer.invoke('window:open-file-detail', id, sequenceIds),
  openSettingsWindow: () => ipcRenderer.invoke('window:open-settings'),
  openRatingManagerWindow: () => ipcRenderer.invoke('window:open-rating-manager'),
  ...
  openEHentaiImportWindow: () => ipcRenderer.invoke('window:open-ehentai-import'),
  openAiManagerWindow: () => ipcRenderer.invoke('window:open-ai-manager'),
  openTagTranslationWindow: () => ipcRenderer.invoke('window:open-tag-translation'),
  ...
  openFileExternally: (fileId) => ipcRenderer.invoke('file:open-externally', fileId),
  ...
  updateNetworkSettings: (settings) => ipcRenderer.invoke('settings:update-network', settings),
  selectStorageDirectory: () => ipcRenderer.invoke('settings:select-storage-directory'),
  updateFileStoragePath: (path) => ipcRenderer.invoke('settings:update-file-storage-path', path),
  updateThumbnailStoragePath: (path) => ipcRenderer.invoke('settings:update-thumbnail-storage-path', path),
  updateConvertImportedImagesToPng: (enabled) =>
    ipcRenderer.invoke('settings:update-convert-imported-images-to-png', enabled),
  preloadThumbnails: (fileIds) => ipcRenderer.invoke('thumbnail:preload', fileIds),
  ...
  getPageLayoutTemplate: (kind) => ipcRenderer.invoke('page-layout:get-template', kind),
  savePageLayoutConfig: (name, layoutJson) => ipcRenderer.invoke('page-layout:save-config', name, layoutJson),
  createPageLayoutConfig: () => ipcRenderer.invoke('page-layout:create-config'),
  renamePageLayoutConfig: (id, name) => ipcRenderer.invoke('page-layout:rename-config', id, name),
  deletePageLayoutConfig: (id) => ipcRenderer.invoke('page-layout:delete-config', id),
  openPageLayoutConfig: (id) => ipcRenderer.invoke('page-layout:open-config', id),
  setDefaultPageLayoutConfig: (id) => ipcRenderer.invoke('page-layout:set-default-config', id),
  setNewPageLayoutConfig: (id) => ipcRenderer.invoke('page-layout:set-new-page-config', id),
  ...
  deleteFilesPermanently: (fileIds) => ipcRenderer.invoke('trash:delete-files-permanently', fileIds),
  setFilesDomain: (fileIds, domain) => ipcRenderer.invoke('file:set-domain', fileIds, domain),
  listDomains: () => ipcRenderer.invoke('domain:list'),
  listFileUrls: (fileIds) => ipcRenderer.invoke('url:list-file-urls', fileIds),
  addFileUrl: (fileIds, url) => ipcRenderer.invoke('url:add-file-url', fileIds, url),
  updateFileUrl: (fileIds, urlId, previousUrl, nextUrl) =>
    ipcRenderer.invoke('url:update-file-url', fileIds, urlId, previousUrl, nextUrl),
  removeFileUrl: (fileIds, urlId, url) => ipcRenderer.invoke('url:remove-file-url', fileIds, urlId, url),
  listRatingGroups: () => ipcRenderer.invoke('rating:list-groups'),
  createRatingGroup: (name) => ipcRenderer.invoke('rating:create-group', name),
  renameRatingGroup: (groupId, name) => ipcRenderer.invoke('rating:rename-group', groupId, name),
  setRatingGroupActive: (groupId, active) => ipcRenderer.invoke('rating:set-group-active', groupId, active),
  deleteRatingGroup: (groupId) => ipcRenderer.invoke('rating:delete-group', groupId),
  listRatingEntries: (groupId) => ipcRenderer.invoke('rating:list-entries', groupId),
  createRatingEntry: (groupId, label, color) => ipcRenderer.invoke('rating:create-entry', groupId, label, color),
  updateRatingEntry: (entryId, label, color) => ipcRenderer.invoke('rating:update-entry', entryId, label, color),
  deleteRatingEntry: (entryId) => ipcRenderer.invoke('rating:delete-entry', entryId),
  reorderRatingEntries: (groupId, entryIds) => ipcRenderer.invoke('rating:reorder-entries', groupId, entryIds),
  ...
  listBatchFileTags: (fileIds) => ipcRenderer.invoke('tag:list-batch-file-tags', fileIds),
  ...
  renameTagStyle: (styleId, name) => ipcRenderer.invoke('tag:rename-style', styleId, name),
  setActiveTagStyle: (styleId) => ipcRenderer.invoke('tag:set-active-style', styleId),
  ...
  createManagedTag: (styleId, tag) => ipcRenderer.invoke('tag:create-managed-tag', styleId, tag),
  deleteManagedTag: (tagId) => ipcRenderer.invoke('tag:delete-managed-tag', tagId),
  deleteManagedTags: (tagIds) => ipcRenderer.invoke('tag:delete-managed-tags', tagIds),
  addFileTags: (fileId, tags) => ipcRenderer.invoke('tag:add-file-tags', fileId, tags),
  removeFileTags: (fileId, tagIds) => ipcRenderer.invoke('tag:remove-file-tags', fileId, tagIds),
  addTagsToFiles: (fileIds, tags) => ipcRenderer.invoke('tag:add-tags-to-files', fileIds, tags),
  removeTagsFromFiles: (fileIds, tagIds) => ipcRenderer.invoke('tag:remove-tags-from-files', fileIds, tagIds),
  ...
  commitImportQueue: (queueIds, confirmedDuplicateQueueIds) =>
    ipcRenderer.invoke('import:commit-queue', queueIds, confirmedDuplicateQueueIds),
  removeImportQueueFiles: (queueIds) => ipcRenderer.invoke('import:remove-queue-files', queueIds),
  clearImportQueue: () => ipcRenderer.invoke('import:clear-queue'),
  testHydrusConnection: (options) => ipcRenderer.invoke('hydrus:test-connection', options),
  ...
  updateHydrusImportSettings: (settings) => ipcRenderer.invoke('hydrus:update-settings', settings),
  testEHentaiGallery: (options) => ipcRenderer.invoke('ehentai:test-gallery', options),
  ...
  updateEHentaiImportSettings: (settings) => ipcRenderer.invoke('ehentai:update-settings', settings),
  getTagTranslationSettings: () => ipcRenderer.invoke('tag-translation:get-settings'),
  updateTagTranslationSettings: (settings) => ipcRenderer.invoke('tag-translation:update-settings', settings),
  selectTagTranslationCsv: () => ipcRenderer.invoke('tag-translation:select-csv'),
  translateFileTags: (fileIds) => ipcRenderer.invoke('tag-translation:translate-files', fileIds),
  ...
  detectAiModel: (modelPath) => ipcRenderer.invoke('ai:detect-model', modelPath),
  detectAiModels: (modelPath, selectedModelName) =>
    ipcRenderer.invoke('ai:detect-models', modelPath, selectedModelName),
  downloadDefaultAiModel: (modelPath) => ipcRenderer.invoke('ai:download-default-model', modelPath),
  tagFilesWithAi: (fileIds, overwrite) => ipcRenderer.invoke('ai:tag-files', fileIds, overwrite),
  ...
  updateApiService: (serviceId, draft) => ipcRenderer.invoke('api:update-service', serviceId, draft),
  deleteApiService: (serviceId) => ipcRenderer.invoke('api:delete-service', serviceId),
  getApiServiceAvailability: (serviceId) => ipcRenderer.invoke('api:get-service-availability', serviceId),
  confirmDialog: (options) => ipcRenderer.invoke('dialog:confirm', options),
  getDialogState: (dialogId) => ipcRenderer.invoke('dialog:get-state', dialogId),
  resizeDialog: (dialogId, width, height) => ipcRenderer.invoke('dialog:resize', dialogId, width, height),
  resolveDialog: (dialogId, confirmed) => ipcRenderer.invoke('dialog:resolve', dialogId, confirmed),
  ...
  onImportProgress: (listener) => {
    const wrappedListener = (_event: Electron.IpcRendererEvent, progress: unknown): void => {
  ...
  onFileDetailReset: (listener) => {
    const wrappedListener = (_event: Electron.IpcRendererEvent, fileId: unknown): void => {
  ...
  onFileFavoriteChanged: (listener) => {
    const wrappedListener = (_event: Electron.IpcRendererEvent, fileId: unknown, favorite: unknown): void => {
  ...
  onWorkStatusChanged: (listener) => {
    const wrappedListener = (_event: Electron.IpcRendererEvent, status: unknown): void => {
