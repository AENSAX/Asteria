  ...
  return (
    <section className="grid h-full min-h-0 min-w-0 grid-rows-[auto_auto_auto_minmax(0,1fr)] bg-(--panel)">
      <header className="flex h-[30px] items-center gap-1.5 border-b border-(--line) bg-(--panel-strong) px-1.5">
        <button className="h-[22px] min-w-[66px] cursor-default border border-(--line-strong) bg-(--surface-bg) px-2 text-[11px]" disabled={readyCount === 0} type="button" onClick={() => onCommitQueue(queueFiles)}>
          现在导入
        </button>
        <button className="h-[22px] min-w-[66px] cursor-default border border-(--line-strong) bg-(--surface-bg) px-2 text-[11px]" disabled={queueFiles.length === 0} type="button" onClick={onCancelQueue}>
          取消
        </button>
      </header>

      <div className="border-b border-(--line) p-1.5">
        <div className="h-6 leading-6 text-(--muted)">{dragActive ? '松开导入媒体文件或文件夹' : progress.message}</div>
      ...
      <dl className="grid grid-cols-[repeat(6,minmax(96px,1fr))] text-(--ink)">
        ...
        <div className="col-span-6 grid min-h-6 grid-cols-[56px_minmax(0,1fr)] border-b border-r border-(--line) bg-(--surface-inset-bg)">
          <dt className="flex min-w-0 items-center px-2 text-(--muted)">当前文件</dt>
      ...
      <div className="min-h-0 overflow-auto border-t border-(--line) bg-(--surface-inset-bg) text-[11px] text-(--ink)">
        ...
        {queueFiles.length > 0 ? (
          queueFiles.map((file) => (
            <div className="grid min-h-6 grid-cols-[96px_78px_62px_minmax(0,1fr)] border-b border-(--line) bg-(--panel)" key={file.id}>
          ...
          <div className="h-6 px-2 leading-6 text-(--muted)">没有待导入文件</div>
        )}
      </div>
    </section>
  );
