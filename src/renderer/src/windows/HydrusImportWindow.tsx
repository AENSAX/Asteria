...
export function HydrusImportWindow(): JSX.Element {
  ...
  const importing = progress.phase === 'testing' || progress.phase === 'searching' || progress.phase === 'metadata' || progress.phase === 'importing';
  ...
  useEffect(() => {
    ...
    return window.asteria.onHydrusImportProgress((nextProgress) => {
      if (shouldLogProgress(nextProgress)) {
        appendDebug(`${nextProgress.phase} ${nextProgress.processed}/${nextProgress.total} ${nextProgress.message}`);
      }
  ...
  async function saveSettings(): Promise<void> {
    ...
    const settings = await window.asteria.updateHydrusImportSettings(createOptions());
    applySettings(settings);
    appendDebug('配置已保存');
  }
  ...
  async function testConnection(): Promise<void> {
    ...
    if (!window.asteria || typeof window.asteria.testHydrusConnection !== 'function') {
      const failedStatus = createConnectionStatus(false, 'Hydrus 导入 API 不可用，请重启应用');
      setStatus(failedStatus);
      setProgress({ ...idleProgress, phase: 'failed', message: failedStatus.message });
      return;
    }
    ...
    try {
      ...
      const nextStatus = await window.asteria.testHydrusConnection(createOptions());
      appendDebug(`连接${nextStatus.ok ? '成功' : '失败'}：${nextStatus.message}`);
      ...
      appendDebug(`test exception=${error instanceof Error ? error.message : 'unknown'}`);
  ...
  async function startImport(): Promise<void> {
    ...
    if (!window.asteria || typeof window.asteria.importFromHydrus !== 'function') {
      appendDebug('importFromHydrus 不存在');
      setProgress({ ...idleProgress, phase: 'failed', message: 'Hydrus 导入 API 不可用，请重启应用' });
      return;
    }

    await saveSettings();
    appendDebug('点击开始导入');
    appendDebug(`searchTags=${createOptions().searchTags.join(',') || 'system:everything'}`);
    setStatus(null);
    setProgress({ ...idleProgress, phase: 'testing', message: '准备 Hydrus 导入' });
    ...
    try {
      ...
      appendDebug(`import exception=${error instanceof Error ? error.message : 'unknown'}`);
      setProgress({
        ...idleProgress,
        phase: 'failed',
  ...
  function appendDebug(line: string): void {
    setDebugLines((currentLines) => [...currentLines.slice(-120), `${new Date().toLocaleTimeString()} ${line}`]);
  }
  ...
  function shouldLogProgress(nextProgress: HydrusImportProgress): boolean {
    ...
    if (nextProgress.phase === 'failed' || nextProgress.phase === 'canceled' || nextProgress.phase === 'completed') {
      return true;
    }
    ...
    return nextProgress.message.includes('失败') || nextProgress.message.includes('异常') || nextProgress.message.includes('缺少');
  }
...
function createConnectionStatus(ok: boolean, message: string): HydrusConnectionStatus {
  return {
    ok,
    message,
    hydrusVersion: null,
