...
export function loadShortcutSettings(): ShortcutSettings {
  ...
  try {
    ...
    for (const config of shortcutActionConfigs) {
      const definitions = normalizeShortcutDefinitions(
        config.action,
        parsed[config.action],
      );
...
export function matchesShortcutDefinition(
...
export function matchesShortcutKeyRelease(
  shortcut: ShortcutDefinition,
  event: KeyboardEvent,
): boolean {
  const eventKey = event.key.length === 1 ? event.key.toLowerCase() : event.key;
...
function normalizeShortcutKey(key: string): string {
  ...
  return (
    ...
    (trimmed.length === 1 ? trimmed.toLowerCase() : trimmed)
  );
}

function normalizeShortcutDefinitions(
  action: ShortcutAction,
  value: unknown,
): ShortcutDefinition[] {
  ...
  return value
    ...
    .filter(
      (definition): definition is ShortcutDefinition => definition !== null,
    );
}

function normalizeShortcutDefinition(
  action: ShortcutAction,
  value: unknown,
): ShortcutDefinition | null {
