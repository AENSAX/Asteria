...
export async function exportFiles(sender: WebContents, options: ExportOptions): Promise<ExportProgress> {
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    emitExportProgress(sender, createExportProgress(
      normalizedOptions.jobId,
      'exporting',
      files.length,
      index,
    ...
    try {
      const renderedName = await renderExportFileName(normalizedOptions.filenameFormat, file, index);
...
function normalizeExportOptions(options: ExportOptions): ExportOptions {
  ...
  const filenameFormat = typeof options.filenameFormat === 'string' && options.filenameFormat.trim()
    ? options.filenameFormat.trim()
    : '{index}-{hash}';
  const fileIds = Array.isArray(options.fileIds)
    ? [...new Set(options.fileIds.filter((id) => Number.isInteger(id) && id > 0))]
    : [];
...
async function renderExportFileName(format: string, file: ExportFileRecord, index: number): Promise<string> {
...
function renderExportVariable(variable: string, file: ExportFileRecord, index: number, time: string): string {
  if (variable === 'index') {
    return String(index);
  }
...
function appendOriginalExtension(fileName: string, file: ExportFileRecord): string {
...
async function ensureUniqueFileName(
  ...
  while (usedNames.has(candidate.toLowerCase()) || await pathExists(join(directory, candidate))) {
...
function emitExportProgress(sender: WebContents, progress: ExportProgress): void {
