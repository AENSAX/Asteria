...
export function setAiTaggingStatusListener(
...
export async function defaultAiModelExists(
  modelPath: string,
): Promise<boolean> {
  const normalizedPath = modelPath.trim();
...
export async function tagFilesWithAi(
  fileIds: number[],
  overwrite: boolean,
...
export async function tagUntaggedImagesWithAi(
  fileIds: number[],
): Promise<AiTaggingSummary> {
  const normalizedFileIds = normalizeFileIds(fileIds);
  const settings = getAiSettings();
...
async function predictTagsForImage(
  imagePath: string,
  settings: AiSettings,
): Promise<TagDraft[]> {
...
async function createModelBundle(
...
function extractPredictions(
  ...
  for (const index of labels.general) {
    ...
    if (score > generalThreshold) {
      predictions.push(
        buildTagPrediction(labels.names[index] ?? "", score, "general"),
      );
    }
  }
  ...
  for (const index of labels.character) {
    ...
    if (score > characterThreshold) {
      predictions.push(
        buildTagPrediction(labels.names[index] ?? "", score, "character"),
      );
    }
  }
...
function compareModelFileName(left: string, right: string): number {
  const leftPriority =
    left === DEFAULT_MODEL_FILE_NAME || left === "model.onnx" ? 0 : 1;
  const rightPriority =
    right === DEFAULT_MODEL_FILE_NAME || right === "model.onnx" ? 0 : 1;
