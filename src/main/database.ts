...
const CONVERT_IMPORTED_IMAGES_TO_PNG_SETTING_KEY = 'convert_imported_images_to_png';
...
const apiPermissions: ApiPermissionRecord[] = [
  { id: 'status.read', name: '读取状态', description: 'GET /api/status' },
  {
    id: 'files.read',
    name: '读取文件',
    description: 'GET /api/files, GET /api/files/:identifier, POST /api/files/duplicates'
  },
  {
    id: 'files.write',
    name: '写入文件信息',
...
export function initializeDatabase(): void {
  ...
  const dataDir = configuredDatabasePath ? dirname(configuredDatabasePath) : join(app.getPath('userData'), 'data');
...
export function listDatabaseFiles(page: number, pageSize = 20): DatabaseFilePage {
  const db = getDatabase();
...
export function searchHints(query: string, limit = 16): SearchHintRecord[] {
  ...
  if (favoriteAliases.some((alias) => normalizeTagSearchQuery(alias).includes(normalizedQuery))) {
    const row = db
      .prepare('SELECT COUNT(*) AS count FROM files WHERE deleted_at IS NULL AND is_favorite = 1')
  ...
  const domainHints = listDomains()
    .filter((domain) => normalizeTagSearchQuery(domain.displayName).includes(normalizedQuery))
    .map((domain) => ({
      id: domain.id === FILE_DOMAIN_PENDING ? -1 : domain.id === FILE_DOMAIN_LIBRARY ? -2 : -3,
...
export function getFileOriginalPath(id: number): string | null {
  const db = getDatabase();
  const row = db.prepare('SELECT storage_path AS storagePath, original_path AS originalPath FROM files WHERE id = ?').get(id) as
...
export function getFileThumbnailSource(id: number): { sourcePath: string; sha256: string; extension: string | null } | null {
  const db = getDatabase();
  const row = db
    .prepare(
      `SELECT
    ...
    .get(id) as
    | { sha256: string; extension: string | null; storagePath: string | null; originalPath: string }
    | undefined;
...
export function listFilesForExport(fileIds: number[]): ExportFileRecord[] {
  const db = getDatabase();
  const normalizedFileIds = [...new Set(fileIds.filter((id) => Number.isInteger(id) && id > 0))];
...
function listFileRatingsForExport(db: Database.Database, fileId: number): FileRatingRecord[] {
  return db
    .prepare(
      `SELECT
...
export function hasStoredFileReference(sha256: string): boolean {
  const db = getDatabase();
  const row = db.prepare('SELECT id FROM files WHERE sha256 = ? LIMIT 1').get(sha256) as
    | { id: number }
    | undefined;
...
export function hasStoredPathReference(storagePath: string): boolean {
  const db = getDatabase();
  const row = db.prepare('SELECT id FROM files WHERE storage_path = ? LIMIT 1').get(storagePath) as
    | { id: number }
    | undefined;
...
export function setTagTranslationSettings(settings: TagTranslationSettings): TagTranslationSettings {
  const normalizedSettings = normalizeTagTranslationSettings(settings);
  setAppSetting(TAG_TRANSLATION_SETTINGS_KEY, JSON.stringify(normalizedSettings));
  return normalizedSettings;
}
...
export function setHydrusImportSettings(settings: HydrusImportOptions): HydrusImportOptions {
...
export function setEHentaiImportSettings(settings: EHentaiImportOptions): EHentaiImportOptions {
  const normalizedSettings = normalizeEHentaiImportSettings(settings);
  setAppSetting(EHENTAI_IMPORT_SETTINGS_KEY, JSON.stringify(normalizedSettings));
  return normalizedSettings;
}
...
export function getApiFileByIdentifier(apiIdentifier: string): ApiFileRecord | null {
  const db = getDatabase();
  const row = db
    .prepare(
      `SELECT
    ...
    .get(apiIdentifier) as (Omit<ApiFileRecord, 'urls' | 'tags' | 'ratings'> & { internalId: number }) | undefined;
...
export function getInternalFileIdByApiIdentifier(apiIdentifier: string): number | null {
  const db = getDatabase();
  const row = db
...
export function updateApiFileMetadata(
  ...
  db.transaction(() => {
    if (shouldUpdateTags) {
      const normalizedTags = expandTagDraftsWithTranslation(normalizeTagDrafts(input.tags ?? []));
      ...
      for (const tag of normalizedTags) {
        const tagId = tag.id
          ? readTagId(db, tag.id)
          : ensureTag(db, styleId, ensureTagNamespace(db, styleId, tag.namespace), tag);

        db.prepare('INSERT OR IGNORE INTO file_tags (file_id, tag_id) VALUES (?, ?)').run(row.id, tagId);
      }
    }
    ...
    if (shouldUpdateTags || shouldUpdateUrls) {
      db.prepare('UPDATE files SET updated_at = datetime(\'now\') WHERE id = ?').run(row.id);
    }
  })();
...
export function createApiUploadedFileRecord(input: ApiUploadedFileRecordInput): ApiFileRecord {
  const db = getDatabase();
...
export function listApiServices(): ApiServiceRecord[] {
  ...
  const rows = db
    ...
    .all() as Array<Omit<ApiServiceRecord, 'enabled' | 'permissions'> & { enabled: number }>;
  const permissionsByServiceId = readApiPermissionsByServiceId(db, rows.map((row) => row.id));
...
export function updateApiService(serviceId: number, draft: ApiServiceDraft): ApiServiceRecord[] {
  const db = getDatabase();
  ...
  db.transaction(() => {
    ...
    db.prepare('DELETE FROM api_service_permissions WHERE service_id = ?').run(serviceId);
...
export function getApiServiceAvailability(serviceId: number): ApiServiceAvailability {
  ...
  const hasPortConflict = services.some((item) =>
    item.id !== service.id &&
    item.enabled &&
    item.address.trim().toLowerCase() === service.address.trim().toLowerCase() &&
    item.port === service.port
  );

  if (hasPortConflict) {
    return createApiServiceAvailability(service, false, '地址和端口已被其他启用 API 使用');
  }
...
function readApiPermissionsByServiceId(
  ...
  const allowedPermissions = new Set(apiPermissions.map((permission) => permission.id));
...
function normalizeApiServiceName(name: string): string {
  ...
  return normalizedName || `API 服务 ${new Date().toISOString().replace(/[:.]/g, '-')}`;
}

function normalizeApiServiceDraft(draft: ApiServiceDraft): ApiServiceDraft {
  const allowedPermissions = new Set(apiPermissions.map((permission) => permission.id));
  const permissions = [...new Set(
    (Array.isArray(draft.permissions) ? draft.permissions : [])
      .filter((permissionId) => allowedPermissions.has(permissionId))
  )];

  return {
    name: normalizeApiServiceName(draft.name),
    address: typeof draft.address === 'string' && draft.address.trim() ? draft.address.trim() : '127.0.0.1',
    port: Number.isInteger(draft.port) ? Math.min(65535, Math.max(1, draft.port)) : 17321,
...
export function getFileStoragePath(): string {
  const db = getDatabase();
  const row = db.prepare('SELECT value FROM app_settings WHERE key = ?').get(FILE_STORAGE_SETTING_KEY) as
    | { value: string }
    | undefined;
...
export function getThumbnailStoragePath(): string {
  const db = getDatabase();
  const row = db.prepare('SELECT value FROM app_settings WHERE key = ?').get(THUMBNAIL_STORAGE_SETTING_KEY) as
    | { value: string }
    | undefined;
...
function normalizeAiSettings(value: unknown): AiSettings {
  ...
  return {
    ...
    autoTagUntaggedImagesOnImport: settings?.autoTagUntaggedImagesOnImport === true,
    enableImageRetagContextMenu: settings?.enableImageRetagContextMenu === true,
    enableImageAppendTagContextMenu: settings?.enableImageAppendTagContextMenu === true
  };
}
...
function normalizeHydrusImportSettings(value: unknown): HydrusImportOptions {
  ...
  return {
    ...
    limit: Number.isFinite(limit) && limit > 0 ? Math.floor(limit) : defaultHydrusImportSettings.limit,
    metadataBatchSize:
...
function normalizeTagTranslationSettings(value: unknown): TagTranslationSettings {
  ...
  return {
    ...
    enableContextMenuTranslation: settings?.enableContextMenuTranslation === true,
...
function toBrowserFileRecord<T extends DatabaseFileRecord>(row: T): T & { mediaUrl: string; thumbnailUrl: string } {
...
): Array<T & { mediaUrl: string; thumbnailUrl: string; ratings: FileRatingRecord[] }> {
  if (files.length === 0) {
    return [];
  }
...
export function listDomains(): Array<{ id: FileDomain; name: string; displayName: string; fileCount: number }> {
  const db = getDatabase();
  const rows = db
    .prepare(
      `SELECT
...
export function setConvertImportedImagesToPng(enabled: boolean): StorageSettings {
  const db = getDatabase();
  setSettingValue(db, CONVERT_IMPORTED_IMAGES_TO_PNG_SETTING_KEY, enabled ? 'true' : 'false');
  return getStorageSettings();
}
...
export function updateFileStorageRecordPath(id: number, storagePath: string, fileName: string): void {
  const db = getDatabase();
...
export function searchTags(query: string, limit = 12): TagRecord[] {
  ...
  return db
    ...
    .all(likeQuery, likeQuery, likeQuery, prefixQuery, prefixQuery, limit) as TagRecord[];
}
...
export function setActiveTagStyle(styleId: number): TagStyleRecord[] {
  ...
  const style = db.prepare('SELECT id FROM tag_styles WHERE id = ?').get(styleId) as
    | { id: number }
    | undefined;
  ...
  db.transaction(() => {
    db.prepare('UPDATE tag_styles SET is_default = 0, updated_at = datetime(\'now\') WHERE is_default = 1').run();
    db.prepare('UPDATE tag_styles SET is_default = 1, updated_at = datetime(\'now\') WHERE id = ?').run(styleId);
  })();
...
export function renameTagStyle(styleId: number, name: string): TagStyleRecord[] {
  const db = getDatabase();
  const displayName = name.trim().replace(/\s+/g, ' ');
  const normalizedName = normalizeTagPart(displayName).replace(/\s+/g, '-');

  if (!Number.isInteger(styleId) || styleId <= 0 || !displayName || !normalizedName) {
    throw new Error('标签风格无效');
  }
...
export function deleteTagStyle(styleId: number): DeleteTagStyleResult {
  ...
  const style = db.prepare('SELECT id FROM tag_styles WHERE id = ?').get(styleId) as { id: number } | undefined;

  if (!style) {
    throw new Error('标签风格不存在');
  }

  const tagCount = db.prepare('SELECT COUNT(*) AS count FROM tags WHERE style_id = ?').get(styleId) as
    | { count: number }
    | undefined;
  ...
  db.transaction(() => {
    ...
    const defaultStyle = db.prepare('SELECT id FROM tag_styles WHERE is_default = 1 LIMIT 1').get() as
      | { id: number }
      | undefined;

    if (!defaultStyle) {
      const fallback = db.prepare('SELECT id FROM tag_styles ORDER BY id ASC LIMIT 1').get() as
        | { id: number }
        | undefined;

      if (fallback) {
        db.prepare('UPDATE tag_styles SET is_default = 1, updated_at = datetime(\'now\') WHERE id = ?').run(fallback.id);
      } else {
        getDefaultTagStyleId(db);
      }
    }
...
export function createManagedTag(styleId: number, tag: TagDraft): ManagedTagRecord {
  const db = getDatabase();
  ...
  const existing = db
    .prepare('SELECT id FROM tags WHERE style_id = ? AND namespace = ? AND name = ?')
    .get(styleId, primaryTag.namespace, primaryTag.name) as { id: number } | undefined;
...
export function deleteManagedTags(tagIds: number[]): DeleteManagedTagsResult {
  ...
  const fileCount = db
    .prepare(`SELECT COUNT(DISTINCT file_id) AS count FROM file_tags WHERE tag_id IN (${placeholders})`)
  ...
  db.transaction(() => {
    db.prepare(`DELETE FROM file_tags WHERE tag_id IN (${placeholders})`).run(...normalizedTagIds);
    db.prepare(`DELETE FROM tags WHERE id IN (${placeholders})`).run(...normalizedTagIds);
  })();
...
export function addFileTags(fileId: number, tags: TagDraft[]): FileTagRecord[] {
  const db = getDatabase();
  const normalizedTags = expandTagDraftsWithTranslation(normalizeTagDrafts(tags));
  ...
  db.transaction(() => {
    const file = db.prepare('SELECT id FROM files WHERE id = ?').get(fileId) as { id: number } | undefined;
    ...
    for (const tag of normalizedTags) {
      const tagId = tag.id ? readTagId(db, tag.id) : ensureTag(db, styleId, ensureTagNamespace(db, styleId, tag.namespace), tag);
...
export function replaceFileTags(fileId: number, tags: TagDraft[]): FileTagRecord[] {
  const db = getDatabase();
  const normalizedTags = expandTagDraftsWithTranslation(normalizeTagDrafts(tags));
...
export function removeFileTags(fileId: number, tagIds: number[]): FileTagRecord[] {
  const db = getDatabase();
  const normalizedTagIds = normalizeTagIds(tagIds);
...
export function listTrashedFiles(page: number, pageSize = 20): DatabaseFilePage {
  const db = getDatabase();
  const normalizedPage = Number.isInteger(page) && page > 0 ? page : 1;
  const offset = (normalizedPage - 1) * pageSize;
...
export function deleteFilesPermanently(fileIds: number[]): DatabaseFileRecord[] {
  const db = getDatabase();
  const normalizedFileIds = normalizeFileIds(fileIds);
  ...
  db.prepare(`DELETE FROM files WHERE id IN (${placeholders})`).run(...normalizedFileIds);
...
export function updateFileUrl(
  ...
  db.transaction(() => {
    ...
    if (normalizedFileIds.length === 1 && Number.isInteger(urlId) && urlId > 0) {
      db.prepare(
        `UPDATE file_urls
         SET url = ?,
             normalized_url = ?,
...
export function removeFileUrl(fileIds: number[], urlId: number, url: string): FileUrlRecord[] {
  const db = getDatabase();
  const normalizedFileIds = normalizeFileIds(fileIds);
  const normalizedUrl = normalizeUrl(url);
  ...
  db.transaction(() => {
    if (normalizedFileIds.length === 1 && Number.isInteger(urlId) && urlId > 0) {
      db.prepare('DELETE FROM file_urls WHERE id = ? AND file_id = ?').run(urlId, normalizedFileIds[0]);
      return;
    }
...
export function renameRatingGroup(groupId: number, name: string): RatingGroupRecord[] {
  const db = getDatabase();
  const normalizedName = name.trim();
...
export function setRatingGroupActive(groupId: number, active: boolean): RatingGroupRecord[] {
  const db = getDatabase();
...
export function createRatingEntry(groupId: number, label: string, color: string): RatingEntryRecord[] {
  const db = getDatabase();
  ...
  const row = db.prepare('SELECT coalesce(MAX(sort_order), 0) + 1 AS sortOrder FROM rating_entries WHERE group_id = ?').get(groupId) as
    | { sortOrder: number }
    | undefined;
...
export function updateRatingEntry(entryId: number, label: string, color: string): RatingEntryRecord[] {
  const db = getDatabase();
  const normalizedLabel = label.trim();
  const normalizedColor = normalizeRatingColor(color);
  ...
  const row = db.prepare('SELECT group_id AS groupId FROM rating_entries WHERE id = ?').get(entryId) as
    | { groupId: number }
    | undefined;
...
export function reorderRatingEntries(groupId: number, entryIds: number[]): RatingEntryRecord[] {
  const db = getDatabase();
  const normalizedEntryIds = normalizePositiveIds(entryIds);

  if (!Number.isInteger(groupId) || groupId <= 0 || normalizedEntryIds.length === 0) {
    return listRatingEntries(groupId);
  }

  const placeholders = createPlaceholders(normalizedEntryIds.length);
  const validRows = db
    .prepare(`SELECT id FROM rating_entries WHERE group_id = ? AND id IN (${placeholders})`)
...
export function setFileRatingEntries(fileIds: number[], groupId: number, entryIds: number[]): void {
  const db = getDatabase();
  const normalizedFileIds = normalizeFileIds(fileIds);
  const normalizedEntryIds = normalizePositiveIds(entryIds);

  if (normalizedFileIds.length === 0 || !Number.isInteger(groupId) || groupId <= 0) {
    return;
  }

  const filePlaceholders = createPlaceholders(normalizedFileIds.length);
  const entryPlaceholders = normalizedEntryIds.length > 0 ? createPlaceholders(normalizedEntryIds.length) : '';
  const validEntryRows =
    normalizedEntryIds.length > 0
      ? (db
          .prepare(`SELECT id FROM rating_entries WHERE group_id = ? AND id IN (${entryPlaceholders})`)
          .all(groupId, ...normalizedEntryIds) as Array<{ id: number }>)
      : [];
  ...
  db.transaction(() => {
    ...
    for (const fileId of normalizedFileIds) {
      for (const entryId of validEntryIds) {
        db.prepare('INSERT OR IGNORE INTO file_ratings (file_id, entry_id) VALUES (?, ?)').run(fileId, entryId);
      }
    }
  })();
}
...
export function addTagsToFiles(fileIds: number[], tags: TagDraft[]): BatchFileTagRecord[] {
  const db = getDatabase();
  const normalizedFileIds = normalizeFileIds(fileIds);
  const normalizedTags = expandTagDraftsWithTranslation(normalizeTagDrafts(tags));
...
export function removeTagsFromFiles(fileIds: number[], tagIds: number[]): BatchFileTagRecord[] {
  const db = getDatabase();
  const normalizedFileIds = normalizeFileIds(fileIds);
  const normalizedTagIds = normalizeTagIds(tagIds);
  ...
  db.transaction(() => {
    ...
    for (const fileId of normalizedFileIds) {
      for (const row of rowsByFileId.get(fileId) ?? []) {
        ...
        db.prepare('INSERT OR IGNORE INTO file_tags (file_id, tag_id) VALUES (?, ?)').run(row.fileId, translatedTagId);
        summary.translatedTagCount += 1;

        if (translatedTagId !== row.tagId) {
          db.prepare('DELETE FROM file_tags WHERE file_id = ? AND tag_id = ?').run(row.fileId, row.tagId);
    ...
    if (!settings.keepOriginalTags && originalTagIds.size > 0) {
      ...
      const originalTagPlaceholders = createPlaceholders(originalTagIdList.length);
    ...
    if (summary.translatedTagCount > 0 || summary.removedOriginalTagCount > 0) {
      db.prepare(`UPDATE files SET updated_at = datetime('now') WHERE id IN (${filePlaceholders})`).run(...normalizedFileIds);
    }
  })();
...
function normalizeTagDrafts(tags: TagDraft[]): TagDraft[] {
  ...
  for (const tag of tags) {
    const id = Number.isInteger(tag.id) && Number(tag.id) > 0 ? Number(tag.id) : undefined;
...
function readTranslationMap(settings: TagTranslationSettings): Map<string, string> {
...
function readTranslatedTagName(name: string, translationMap: Map<string, string>): string | null {
...
function parseSearchExpression(tokens: SearchToken[]): SearchNode | null {
  ...
  function startsExpression(token: SearchToken | undefined): boolean {
    return token?.kind === 'tag' || token?.kind === 'leftParen' || token?.kind === 'minus';
  }
...
function readSearchTagIndex(): Map<string, Set<number>> {
  ...
  for (const row of tagRows) {
    const keys = [row.name, row.namespace ? `${row.namespace}:${row.name}` : row.name];
  ...
  const ratingRows = db
    ...
    .all() as Array<{ fileId: number; entryId: number; groupName: string; label: string }>;
  ...
  const domainRows = db
    ...
    .all() as Array<{ fileId: number; domain: FileDomain; deletedAt: string | null }>;
...
function addSearchIndexEntry(tagIndex: Map<string, Set<number>>, key: string, fileId: number): void {
...
function evaluateSearchNode(
  ...
  if (node.kind === 'not') {
    return subtractSets(universe, evaluateSearchNode(node.node, universe, tagIndex));
  }
...
function ensureFilesExist(db: Database.Database, fileIds: number[]): void {
  ...
  const placeholders = createPlaceholders(fileIds.length);
  const row = db.prepare(`SELECT COUNT(*) AS count FROM files WHERE id IN (${placeholders})`).get(...fileIds) as
    | { count: number }
    | undefined;
...
function getDefaultTagStyleId(db: Database.Database): number {
  const row = db.prepare('SELECT id FROM tag_styles WHERE is_default = 1 LIMIT 1').get() as
    | { id: number }
    | undefined;
  ...
  const existingDefault = db.prepare('SELECT id FROM tag_styles WHERE name = ?').get('default') as
    | { id: number }
    | undefined;

  if (existingDefault) {
    db.prepare('UPDATE tag_styles SET is_default = 1 WHERE id = ?').run(existingDefault.id);
...
function ensureTagStyleByName(db: Database.Database, value: string): number {
  ...
  const normalizedName = normalizeTagPart(displayName).replace(/\s+/g, '-');
  const existing = db
    .prepare('SELECT id FROM tag_styles WHERE name = ? OR lower(display_name) = lower(?) LIMIT 1')
...
function ensureTagNamespace(db: Database.Database, styleId: number, namespace: string): number {
  const existing = db
  ...
  const result = db
    .prepare('INSERT INTO tag_namespaces (style_id, name, display_name) VALUES (?, ?, ?)')
...
function readTagId(db: Database.Database, tagId: number): number {
  const existing = db.prepare('SELECT id FROM tags WHERE id = ?').get(tagId) as { id: number } | undefined;
...
function readManagedTag(db: Database.Database, tagId: number): ManagedTagRecord | null {
  const row = db
    .prepare(
      `SELECT
        tags.id,
...
function runMigrations(db: Database.Database): void {
  ...
  if (currentVersion < 1) {
    db.transaction(() => {
      ...
      const pageResult = db
        .prepare('INSERT INTO pages (title, kind, layout_json) VALUES (?, ?, ?)')
...
function migrateToApiFileIdentifiers(db: Database.Database): void {
  db.transaction(() => {
    ensureApiFileIdentifiersSchema(db);
    db.prepare('INSERT INTO schema_migrations (version, name) VALUES (?, ?)').run(9, 'api_file_identifiers');
  })();
}
...
function ensureApiFileIdentifiersSchema(db: Database.Database): void {
  ...
  const hasApiIdentifier = columns.some((column) => column.name === 'api_identifier');
  ...
  const rows = db
    .prepare('SELECT id FROM files WHERE api_identifier IS NULL OR api_identifier = ?')
    .all('') as Array<{ id: number }>;

  for (const row of rows) {
    db.prepare('UPDATE files SET api_identifier = ? WHERE id = ?').run(createApiFileIdentifier(), row.id);
  }
...
function migrateToApiServices(db: Database.Database): void {
  db.transaction(() => {
    ...
    db.prepare('INSERT INTO schema_migrations (version, name) VALUES (?, ?)').run(8, 'api_services');
  })();
}
...
function migrateToTagStyles(db: Database.Database): void {
  ...
  try {
    db.transaction(() => {
      ...
      const defaultStyle = db.prepare('SELECT id FROM tag_styles WHERE name = ?').get('default') as
        | { id: number }
        | undefined;
      ...
      db.prepare('INSERT OR IGNORE INTO modules (id, title, kind) VALUES (?, ?, ?)').run(
        'tag-manager',
        '标签管理',
        'tag'
      );

      db.prepare('INSERT INTO schema_migrations (version, name) VALUES (?, ?)').run(2, 'tag_styles_and_file_urls');
    })();
  } finally {
...
function migrateToStorageSettings(db: Database.Database): void {
  db.transaction(() => {
    ...
    setSettingValue(db, THUMBNAIL_STORAGE_SETTING_KEY, getDefaultThumbnailStoragePath());

    db.prepare('INSERT INTO schema_migrations (version, name) VALUES (?, ?)').run(3, 'file_storage_settings');
  })();
}
...
function migrateToFileDomains(db: Database.Database): void {
  db.transaction(() => {
    ...
    db.prepare('INSERT INTO schema_migrations (version, name) VALUES (?, ?)').run(4, 'file_domains');
  })();
}
...
function readFileCount(db: Database.Database, trashed: boolean): number {
  const row = db
    .prepare(`SELECT COUNT(*) as count FROM files WHERE deleted_at IS ${trashed ? 'NOT ' : ''}NULL`)
    .get() as { count: number } | undefined;
