...
type StorageMigrationOutcome = 'migrated' | 'adopted' | 'alreadyCurrent' | 'missing';
type StorageMigrationResult = Record<StorageMigrationOutcome, number>;

function createAsteriaWindow(options: BrowserWindowConstructorOptions): BrowserWindow {
...
function createEHentaiImportWindow(): BrowserWindow {
  const existing = BrowserWindow.getAllWindows().find(
    (window) => window.getTitle() === '从 E-Hentai 导入' && !window.isDestroyed()
  );
...
function createFileRatingEditorWindow(fileIds: number[], groupId: number): BrowserWindow {
...
function createFileDetailWindow(id: number, sequenceIds?: number[]): BrowserWindow {
...
function setFileDetailSequence(window: BrowserWindow, fileId: number, sequenceIds?: number[]): void {
...
function createGenericDialogWindow(state: GenericDialogState, parent?: BrowserWindow | null): BrowserWindow {
  const window = createAsteriaWindow({
  ...
  window.once('closed', () => {
    ...
    if ((state.kind === 'confirm' || state.kind === 'alert') && request.resolve) {
...
function formatChildWindowTasks(childWindows: BrowserWindow[]): string[] {
  ...
  return [...titleCounts.entries()]
    ...
    .map(([title, count]) => `子窗口：${title}${count > 1 ? ` × ${count}` : ''}`);
}
...
function normalizeIpcFileIds(value: unknown): number[] {
  ...
  for (const fileId of value) {
    if (typeof fileId !== 'number' || !Number.isInteger(fileId) || fileId <= 0 || seen.has(fileId)) {
      continue;
    }
...
function normalizeConfirmDialogOptions(value: unknown): ConfirmDialogOptions {
  ...
  return {
    title: typeof options?.title === 'string' && options.title.trim() ? options.title.trim() : '确认',
    message,
    confirmText:
...
function normalizeHydrusImportOptions(value: unknown): HydrusImportOptions {
  ...
  return {
    baseUrl: typeof options?.baseUrl === 'string' ? options.baseUrl : 'http://127.0.0.1:45869',
    accessKey: typeof options?.accessKey === 'string' ? options.accessKey : '',
    searchTags: Array.isArray(options?.searchTags)
      ? options.searchTags.filter((tag): tag is string => typeof tag === 'string')
      : [],
    tagStyleName: typeof options?.tagStyleName === 'string' ? options.tagStyleName : 'hydrus',
    limit: typeof options?.limit === 'number' ? options.limit : 0,
    metadataBatchSize: typeof options?.metadataBatchSize === 'number' ? options.metadataBatchSize : 100,
...
function normalizeEHentaiImportOptions(value: unknown): EHentaiImportOptions {
  ...
  return {
    galleryUrl: typeof options?.galleryUrl === 'string' ? options.galleryUrl : '',
    cookie: typeof options?.cookie === 'string' ? options.cookie : '',
    importGalleryTags: options?.importGalleryTags !== false,
    forceDuplicate: options?.forceDuplicate === true,
    requestDelayMs: 10_000,
    requestTimeoutMs: typeof options?.requestTimeoutMs === 'number' ? options.requestTimeoutMs : 45_000,
    startIndex: typeof options?.startIndex === 'number' ? options.startIndex : 1,
    limit: typeof options?.limit === 'number' ? options.limit : 0
  };
}
...
function normalizeAiSettings(value: unknown): AiSettings {
  ...
  return {
    ...
    characterThreshold: normalizeAiThreshold(settings?.characterThreshold, 0.75),
  ...
  return {
    ...
    proxyHost:
      typeof settings?.proxyHost === 'string'
        ...
        : '',
    proxyPort: Number.isInteger(port) && port > 0 && port <= 65535 ? port : 7890
  };
}
...
function openConfirmDialog(options: ConfirmDialogOptions, parent?: BrowserWindow | null): Promise<boolean> {
...
function openAlertDialog(options: ConfirmDialogOptions, parent?: BrowserWindow | null): Promise<void> {
  const id = createGenericDialogId();
  const state: GenericDialogState = {
    id,
    kind: 'alert',
...
function openProgressDialog(progress: OperationProgress, parent?: BrowserWindow | null): string {
  const id = createGenericDialogId();
  const state: GenericDialogState = {
    id,
    kind: 'progress',
...
async function updateStoragePathWithMigration(
  ...
  if (resolve(currentSettings.fileStoragePath) === resolve(normalizedNextPath)) {
  ...
  try {
    ...
    await clearDirectoryContents(currentSettings.fileStoragePath, normalizedNextPath);
...
async function updateThumbnailStoragePathWithMigration(
  ...
  if (resolve(currentSettings.thumbnailStoragePath) === resolve(normalizedNextPath)) {
    return currentSettings;
  }
  ...
  try {
    const sourceExists = await directoryExists(currentSettings.thumbnailStoragePath);
    ...
    await clearDirectoryContents(currentSettings.thumbnailStoragePath, normalizedNextPath);

    updateProgressDialog(progressDialogId, {
      title: '正在迁移',
      total: 1,
      processed: 1,
      message: sourceExists ? '缩略图缓存迁移完成' : '原缩略图缓存缺失，已使用新位置',
      completed: true
    });
...
async function getPageLayoutTemplate(kind: 'default' | 'newPage'): Promise<string> {
  const settings = getPageLayoutSettings();
  const configId = kind === 'newPage' ? settings.newPageConfigId : settings.defaultConfigId;
...
async function savePageLayoutConfig(name: string, layoutJson: string): Promise<PageLayoutConfigRecord[]> {
  JSON.parse(layoutJson);

  const directory = await ensurePageLayoutDirectory();
  const id = await createUniqueLayoutConfigId(name);
  await writeFile(join(directory, `${id}${PAGE_LAYOUT_FILE_EXTENSION}`), layoutJson, 'utf8');
...
async function renamePageLayoutConfig(id: string, name: string): Promise<PageLayoutConfigRecord[]> {
  const sourcePath = await requirePageLayoutConfigPath(id);
  const nextId = await createUniqueLayoutConfigId(name);
  await rename(sourcePath, join(await ensurePageLayoutDirectory(), `${nextId}${PAGE_LAYOUT_FILE_EXTENSION}`));
...
async function deletePageLayoutConfig(id: string): Promise<PageLayoutConfigRecord[]> {
...
async function setPageLayoutSetting(key: string, id: string | null): Promise<PageLayoutSettings> {
  if (!id) {
...
async function resolvePageLayoutConfigPath(id: string): Promise<string | null> {
  ...
  const path = join(await ensurePageLayoutDirectory(), `${id}${PAGE_LAYOUT_FILE_EXTENSION}`);
...
async function createUniqueLayoutConfigId(name: string): Promise<string> {
  ...
  while (await pathExists(join(directory, `${id}${PAGE_LAYOUT_FILE_EXTENSION}`))) {
    index += 1;
    id = `${baseName}-${index}`;
  }
...
function sanitizeLayoutConfigName(name: string): string {
  const normalizedName = name.trim().replace(/\s+/g, '-').replace(/[<>:"/\\|?*\u0000-\u001f]/g, '');
  return normalizedName || `布局配置-${new Date().toISOString().replace(/[:.]/g, '-')}`;
}
...
function buildStorageMigrationProgressMessage(result: StorageMigrationResult): string {
  return [
  ...
  ].filter(Boolean).join('，') || '正在迁移';
}

function buildStorageMigrationCompleteMessage(result: StorageMigrationResult): string {
...
async function clearDirectoryContents(directoryPath: string, protectedPath: string): Promise<void> {
...
async function deleteStoredFilesPermanently(fileIds: number[]): Promise<void> {
  ...
  for (const file of deletedFiles) {
    if (file.storagePath && !checkedStoragePaths.has(file.storagePath) && !hasStoredFileReference(file.sha256)) {
...
function updateTagTranslationWorkStatus(completed: number, total: number): void {
...
app.whenReady().then(async () => {
  ...
  protocol.handle('asteria-media', async (request) => {
    ...
    const filePath =
      kind === 'import'
        ...
        : kind === 'thumbnail'
          ? (await ensureThumbnailForFile(id, expectedSha256)) ?? getThumbnailFallbackPath(id, expectedSha256)
  ...
  ipcMain.handle('dialog:confirm', (event, options: unknown) =>
    openConfirmDialog(normalizeConfirmDialogOptions(options), BrowserWindow.fromWebContents(event.sender))
  );
  ...
  ipcMain.handle('dialog:resize', (_event, dialogId: unknown, width: unknown, height: unknown) => {
    if (
  ...
  ipcMain.handle('dialog:resolve', (_event, dialogId: unknown, confirmed: unknown) => {
    if (typeof dialogId === 'string') {
  ...
  ipcMain.handle('file:set-favorite', (_event, fileId: unknown, favorite: unknown) => {
  ...
  ipcMain.handle('window:open-batch-tag-manager', (_event, fileIds: unknown) => {
    const normalizedFileIds = normalizeIpcFileIds(fileIds);
  ...
  ipcMain.handle('window:open-file-rating-editor', (_event, fileIds: unknown, groupId: unknown) => {
    const normalizedFileIds = normalizeIpcFileIds(fileIds);

    if (normalizedFileIds.length > 0 && typeof groupId === 'number' && Number.isInteger(groupId) && groupId > 0) {
  ...
  ipcMain.handle('window:open-file-detail', (_event, id: unknown, sequenceIds: unknown) => {
  ...
  ipcMain.handle('file-detail:get-sequence', (event) =>
    fileDetailSequences.get(event.sender.id) ?? []
  );
  ...
  ipcMain.handle('trash:delete-files-permanently', async (_event, fileIds: unknown) => {
    await deleteStoredFilesPermanently(normalizeIpcFileIds(fileIds));
    broadcastFilesChanged();
  });
  ipcMain.handle('file:set-domain', (_event, fileIds: unknown, domain: unknown) => {
  ...
  ipcMain.handle('url:list-file-urls', (_event, fileIds: unknown) => listFileUrls(normalizeIpcFileIds(fileIds)));
  ipcMain.handle('url:add-file-url', (_event, fileIds: unknown, url: unknown) => {
  ...
  ipcMain.handle(
    'url:update-file-url',
    (_event, fileIds: unknown, urlId: unknown, previousUrl: unknown, nextUrl: unknown) => {
      if (
        typeof urlId !== 'number' ||
      ...
      return updateFileUrl(normalizeIpcFileIds(fileIds), urlId, previousUrl, nextUrl);
    }
  );
  ipcMain.handle('url:remove-file-url', (_event, fileIds: unknown, urlId: unknown, url: unknown) => {
  ...
  ipcMain.handle('rating:rename-group', (_event, groupId: unknown, name: unknown) => {
  ...
  ipcMain.handle('rating:set-group-active', (_event, groupId: unknown, active: unknown) => {
  ...
  ipcMain.handle('rating:create-entry', (_event, groupId: unknown, label: unknown, color: unknown) => {
    if (typeof groupId !== 'number' || typeof label !== 'string' || typeof color !== 'string') {
      throw new Error('分级条目无效');
    }

    return createRatingEntry(groupId, label, color);
  });
  ipcMain.handle('rating:update-entry', (_event, entryId: unknown, label: unknown, color: unknown) => {
    if (typeof entryId !== 'number' || typeof label !== 'string' || typeof color !== 'string') {
      throw new Error('分级条目无效');
    }
  ...
  ipcMain.handle('rating:reorder-entries', (_event, groupId: unknown, entryIds: unknown) => {
    ...
    const entries = reorderRatingEntries(
      groupId,
      entryIds.filter((entryId): entryId is number => typeof entryId === 'number')
    );
    broadcastFilesChanged();
    return entries;
  });
  ipcMain.handle('rating:set-file-entries', (_event, fileIds: unknown, groupId: unknown, entryIds: unknown) => {
    if (typeof groupId !== 'number' || !Array.isArray(entryIds)) {
  ...
  ipcMain.handle('settings:update-network', async (_event, settings: unknown) => {
    const nextSettings = setNetworkSettings(normalizeNetworkSettings(settings));
    await applyNetworkSettings(nextSettings);
    return nextSettings;
  });
  ipcMain.handle('settings:select-storage-directory', () => chooseStorageDirectory());
  ...
  ipcMain.handle('api:update-service', async (_event, serviceId: unknown, draft: unknown) => {
    if (
  ...
  ipcMain.handle('api:delete-service', async (_event, serviceId: unknown) => {
    if (typeof serviceId !== 'number' || !Number.isInteger(serviceId) || serviceId <= 0) {
      throw new Error('API 服务无效');
    }
  ...
  ipcMain.handle('api:get-service-availability', (_event, serviceId: unknown) => {
    if (typeof serviceId !== 'number' || !Number.isInteger(serviceId) || serviceId <= 0) {
      return {
        serviceId: 0,
        available: false,
  ...
  ipcMain.handle('settings:update-file-storage-path', (event, path: unknown) => {
  ...
  ipcMain.handle('settings:update-thumbnail-storage-path', (event, path: unknown) => {
    if (typeof path !== 'string') {
      throw new Error('路径无效');
    }
  ...
  ipcMain.handle('settings:update-convert-imported-images-to-png', (_event, enabled: unknown) =>
  ...
  ipcMain.handle('page-layout:save-config', (_event, name: unknown, layoutJson: unknown) => {
  ...
  ipcMain.handle('page-layout:rename-config', (_event, id: unknown, name: unknown) => {
  ...
  ipcMain.handle('page-layout:set-default-config', (_event, id: unknown) => {
    ...
    return (async () => {
      const result = await setPageLayoutSetting(PAGE_LAYOUT_DEFAULT_SETTING_KEY, id);
  ...
  ipcMain.handle('page-layout:set-new-page-config', (_event, id: unknown) => {
    ...
    return (async () => {
      const result = await setPageLayoutSetting(PAGE_LAYOUT_NEW_PAGE_SETTING_KEY, id);
      broadcastPageLayoutChanged();
      return result;
    })();
  });
  ...
  ipcMain.handle('tag:search', (_event, query: unknown) => searchTags(typeof query === 'string' ? query : ''));
  ipcMain.handle('search:hints', (_event, query: unknown) => searchHints(typeof query === 'string' ? query : ''));
  ...
  ipcMain.handle('tag:rename-style', (_event, styleId: unknown, name: unknown) => {
    if (typeof styleId !== 'number' || !Number.isInteger(styleId) || styleId <= 0 || typeof name !== 'string') {
  ...
  ipcMain.handle('tag:set-active-style', (_event, styleId: unknown) => {
    if (typeof styleId !== 'number' || !Number.isInteger(styleId) || styleId <= 0) {
      throw new Error('标签风格无效');
    }
  ...
  ipcMain.handle('tag:list-managed-tags', (_event, styleId: unknown, sortKey: unknown, direction: unknown) => {
    if (typeof styleId !== 'number' || !Number.isInteger(styleId) || styleId <= 0) {
      return [];
    }
  ...
  ipcMain.handle('tag:create-managed-tag', (_event, styleId: unknown, tag: unknown) => {
    if (
  ...
  ipcMain.handle('tag:add-file-tags', (_event, fileId: unknown, tags: unknown) => {
    if (typeof fileId !== 'number' || !Number.isInteger(fileId) || fileId <= 0 || !Array.isArray(tags)) {
      return [];
    }
  ...
  ipcMain.handle('tag:remove-file-tags', (_event, fileId: unknown, tagIds: unknown) => {
    if (typeof fileId !== 'number' || !Number.isInteger(fileId) || fileId <= 0 || !Array.isArray(tagIds)) {
      return [];
    }
  ...
  ipcMain.handle('tag:add-tags-to-files', (_event, fileIds: unknown, tags: unknown) => {
    if (!Array.isArray(tags)) {
      return [];
    }

    const fileTags = addTagsToFiles(normalizeIpcFileIds(fileIds), tags.filter(isTagDraft));
    broadcastFilesChanged();
    return fileTags;
  });
  ipcMain.handle('tag:remove-tags-from-files', (_event, fileIds: unknown, tagIds: unknown) => {
    if (!Array.isArray(tagIds)) {
      return [];
    }
  ...
  ipcMain.handle('hydrus:import', async (event, options: unknown) => {
    const result = await importFromHydrus(event.sender, normalizeHydrusImportOptions(options));
    broadcastFilesChanged();
    queueAllMissingThumbnails('normal');
    return result;
  });
  ...
  ipcMain.handle('ehentai:import', async (event, options: unknown) => {
    await applyNetworkSettings(getNetworkSettings());
    const result = await importFromEHentai(event.sender, normalizeEHentaiImportOptions(options));
    broadcastFilesChanged();
    queueAllMissingThumbnails('normal');
    return result;
  });
  ...
  ipcMain.handle('tag-translation:get-settings', () => getTagTranslationSettings());
  ipcMain.handle('tag-translation:update-settings', (_event, settings: unknown) =>
  ...
  ipcMain.handle('tag-translation:translate-files', (_event, fileIds: unknown) => {
    const normalizedFileIds = normalizeIpcFileIds(fileIds);
    beginTagTranslationWorkStatus(normalizedFileIds.length);

    try {
      const result = translateFileTags(normalizedFileIds, (completed, total) => {
  ...
  ipcMain.handle('ai:detect-models', (_event, modelPath: unknown, selectedModelName: unknown) =>
    detectAiModels(
      typeof modelPath === 'string' ? modelPath : '',
      typeof selectedModelName === 'string' ? selectedModelName : ''
    )
  );
  ipcMain.handle('ai:download-default-model', async (event, modelPath: unknown) => {
    const normalizedPath = typeof modelPath === 'string' ? modelPath.trim() : '';
    ...
    try {
      const result = await downloadDefaultAiModel(normalizedPath, (progress) => {
        ...
        updateProgressDialog(progressDialogId, {
          ...
          processed: Math.min(progress.totalFiles * 100, progress.completedFiles * 100 + filePercent),
  ...
  ipcMain.handle('ai:tag-files', async (event, fileIds: unknown, overwrite: unknown) => {
    const normalizedFileIds = normalizeIpcFileIds(fileIds);
    ...
    const result = await tagFilesWithAi(normalizedFileIds, overwrite === true);
    broadcastFilesChanged();
